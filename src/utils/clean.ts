...
const queryQuarterYear = (quarters: Option[], years: Option[]) => {
    if(quarters.length === 1 && years.length === 1){
        return {quarter: quarters[0].value, year: exactYear(quarters[0].value, years[0].value)}
...
const condenseText = (instructor: string, quarter: string, year: string, department: string, classNumber: string, classCode: string) => {
    let quarter_clean = quarter
    if((instructor || year || department || classNumber || classCode) && quarter == 'All') {
        quarter_clean = ''
    }
...
export const querySelectorText = (query: Query) => {
    ...
    return condenseText(instructor, quarter, year, department, classNumber, classCode)
}