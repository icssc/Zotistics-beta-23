...
const Query = ({name, queryId, color}: QueryProps) => {
    const {states, removeQuery, selectedQuery, setSelectedQuery} = useContext(QueriesContext);

  return (
    <div className="grid min-w-[20%]">
        <div
            className={`flex flex-row justify-between gap-1 items-center cursor-pointer py-2 pr-1 pl-2 text-sm rounded-lg dark:text-slate-100
